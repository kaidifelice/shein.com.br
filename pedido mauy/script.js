...
function updatePanelContent(titleText, paragraphText, buttonsConfig, isForm = false) {
    ...
    if (isForm) {
        ...
        buttonsConfig.forEach(placeholder => {
            let inputType = 'text';
            if (placeholder.includes('Idade') || placeholder.includes('Beleza')) {
                inputType = 'number';
            }
            form.innerHTML += `<input type="${inputType}" placeholder="${placeholder}" min="0" max="10" required>`;
        });
...
const handleAccept = () => {
    updatePanelContent('vai recusar um jantar, airbnb, alcool e uma noite dahora? kk calma, ce disse sim? pelo tanto de nao que ja levei seu, achei que ia receber mais um', '', []); 
    setTimeout(() => {
